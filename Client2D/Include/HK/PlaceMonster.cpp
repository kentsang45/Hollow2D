	...
	for (size_t stageNumber = m_iStart; stageNumber < m_iCount; ++stageNumber)
	{
		...
		if (true == setMonster)
		{
			...
			case 4:
				...
				jump1->SetStageNumber(stageNumber);
				jump1->PlaceAt(28, 30);

				shield1 = m_pScene->SpawnObject<ShieldBug>();
				shield1->SetStageNumber(stageNumber);
				shield1->PlaceAt(16, 30);

				shield2 = m_pScene->SpawnObject<ShieldBug>();
				shield2->SetStageNumber(stageNumber);
